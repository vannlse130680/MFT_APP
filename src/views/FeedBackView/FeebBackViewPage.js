...
import { actFetchFeedbacks, actSearchFeedbacks } from 'actions/feedback';
...
const FeebBackViewPage = () => {
  ...
  const feedBacksStore = useSelector(state => state.feedbacks);
  ...
  useEffect(() => {
    ...
    callAPI(`Feedback/getFeedbackByUsername/${router.match.params.username}`, 'GET', null)
      .then(res => {
        if (res.status === 200) {
          // fetch(
          ...
          dispatch(actSearchFeedbacks(searchValue));
  ...
  const handleSearch = keyword => {
    ...
    dispatch(actSearchFeedbacks(keyword));
  };
  ...
  return (
    <Page className={classes.root} title="Đánh giá">
      <AuthGuard roles={['Quản lý']}></AuthGuard>
      <Header  />
...
export default FeebBackViewPage;
